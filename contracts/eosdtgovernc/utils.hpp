...
static char *write_value(char *r, const char *l, const double v) {
    ...
    auto m = v > 0 ? v : -v;
    m -= (ds_ulong) m;
    if (r + 1 > l || m < 0.000000000001) {
        return r;
    }
    *(r++) = '.';
    return write_ulong(r, l, (ds_ulong) (m * 1000000000000 + 0.999999999999), 12ULL);
}
...
static auto parse_number(const char *c) {
    auto result = 0.0L;
    bool decimals = false;
    auto digits = 0;
    for (;*c && *c != ',' && *c != '}';c++) {
        if (*c > 47 && *c < 58) {
            ...
            result = (long double) *c - 48.0L + result * 10.0L;
        } else if (*c == 46 && !decimals) {
            decimals = true;
        } else {
            if (result != 0.0 || digits != 0 || decimals) {
                if (*c != ' ' && *c != '\n' && *c != '\r' && *c != '\t') {
                    break;
                }
            }
    ...
    for (auto i = 0; i < digits; i++)result /= 10.0L;

    return result;
}

static bool parse_double(const char *c,const int len, double &value) {
    value = 0.0;
    bool decimals = false;
    auto digits = 1.0;
    auto l = c + len;
    for (; c < l && *c != ' '; c++) {
        if (*c > 47 && *c < 58) {
            ...
            value = (double) *c - 48.0 + value * 10.0;
        } else if (*c == 46 && !decimals) {
            decimals = true;
        } else {
            return false;
...
static bool parse_int(const char *c,const int len, int &value) {
    value = 0;
    auto l = c + len;
    for (; c < l; c++) {
        if (*c > 47 && *c < 58) {
            value = (int) *c - 48 + value * 10;
        } else {
            return false;
        }
    }
