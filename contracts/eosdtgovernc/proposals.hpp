...
public:
    ...
    void propose(
        ...
        auto balance = balance_get(proposer, UTILITY_SYMBOL);
        auto min_proposal_weight = op_mul_div(oracle_get_rate(UTILITY_SYMBOL),
                to_ldouble(settings.min_proposal_weight)*already_created,to_ldouble(oracle_get_rate(USD_SYMBOL)));
        ...
        ds_assert(validate_proposal_json(proposal_json.c_str()), "proposal_json is invalid");
